...
async def save_media_in_channel_local(bot: Client, editable: Message, message: Message):
    try:
        forwarded_msg = await message.forward(Config.DB_CHANNEL)
        file_er_id = str(forwarded_msg.id)
        await forwarded_msg.reply_text(f"#PRIVATE_FILE:\n\nGenerated by [{message.from_user.first_name}](tg://user?id={message.from_user.id})", disable_web_page_preview=True)
        
        base_share_link = f"https://telegram.me/{Config.BOT_USERNAME}?start=VJBotz_{str_to_b64(file_er_id)}"
        short_link = await get_shortlink(base_share_link)
        ...
        await save_media_in_channel_local(bot, editable, message)
    except Exception as err:
        await editable.edit(f"Something Went Wrong!\n\n**Error:** `{err}`")

async def save_batch_media_in_channel_local(bot: Client, editable: Message, message_ids: list):
    try:
        string_ids = " ".join(str(x) for x in message_ids)
        inserted_id = await db.add_batch_links(string_ids)
        
        base_share_link = f"https://telegram.me/{Config.BOT_USERNAME}?start=BATCH_{str_to_b64(str(inserted_id))}"
        short_link = await get_shortlink(base_share_link)
        
        await editable.edit(
            f"**Your Batch Files Stored in my Database!**\n\n"
            f"**Here is the Permanent Link of your batch files:** {short_link}\n\n"
...
async def main(bot: Client, message: Message):
    if message.chat.type == enums.ChatType.PRIVATE:
        ...
        try:
            forwarded_msg = await message.forward(Config.DB_CHANNEL)
            file_er_id = str(forwarded_msg.id)
            base_share_link = f"https://t.me/{Config.BOT_USERNAME}?start=VJBotz_{str_to_b64(file_er_id)}"
            share_link = await get_shortlink(base_share_link)
            CH_edit = await bot.edit_message_reply_markup(message.chat.id, message.id, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Get Sharable Link", url=share_link)]]))
            ...
            await bot.send_message(chat_id=int(Config.LOG_CHANNEL), text=f"#FloodWait:\nGot FloodWait of `{str(sl.value)}s` from `{str(message.chat.id)}` !!", disable_web_page_preview=True)
        except Exception as err:
            await bot.leave_chat(message.chat.id)
            await bot.send_message(chat_id=int(Config.LOG_CHANNEL), text=f"#ERROR_TRACEBACK:\nGot Error from `{str(message.chat.id)}` !!\n\n**Traceback:** `{err}`", disable_web_page_preview=True)
...
async def ban(c: Client, m: Message):
    if len(m.command) == 1:
        await m.reply_text(f"Use this command to ban any user from the bot.\n\nUsage:\n\n`/ban_user user_id ban_duration ban_reason`\n\nEg: `/ban_user 1234567 28 You misused me.`\nThis will ban user with id `1234567` for `28` days for the reason `You misused me`.", quote=True)
        return
    try:
        ...
        traceback.print_exc()
        await m.reply_text(f"Error occoured! Traceback given below\n\n`{traceback.format_exc()}`", quote=True)

@Bot.on_message(filters.private & filters.command("unban_user") & filters.user(Config.BOT_OWNER))
async def unban(c: Client, m: Message):
    if len(m.command) == 1:
        await m.reply_text(f"Use this command to unban any user.\n\nUsage:\n\n`/unban_user user_id`\n\nEg: `/unban_user 1234567`\nThis will unban user with id `1234567`.", quote=True)
        return
    try:
        user_id = int(m.command[1])
...
async def button(bot: Client, cmd: CallbackQuery):
    ...
    if "aboutbot" in cb_data:
        await cmd.message.edit(Config.ABOUT_BOT_TEXT, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Source Codes of Bot", url="https://youtube.com/@Tech_VJ")], [InlineKeyboardButton("Go Home", callback_data="gotohome"), InlineKeyboardButton("About Dev", callback_data="aboutdevs")]]))
    elif "aboutdevs" in cb_data:
        await cmd.message.edit(Config.ABOUT_DEV_TEXT, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Source Codes of Bot", url="https://youtube.com/@Tech_VJ")], [InlineKeyboardButton("About Bot", callback_data="aboutbot"), InlineKeyboardButton("Go Home", callback_data="gotohome")]]))
    elif "gotohome" in cb_data:
        await cmd.message.edit(Config.HOME_TEXT.format(cmd.message.chat.first_name, cmd.message.chat.id), disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Updates Channel", url="https://t.me/VJ_Botz")], [InlineKeyboardButton("About Bot", callback_data="aboutbot"), InlineKeyboardButton("About Dev", callback_data="aboutdevs"), InlineKeyboardButton("Close 🚪", callback_data="closeMessage")], [InlineKeyboardButton("Support Group", url="https://t.me/VJ_Bot_Disscussion"), InlineKeyboardButton("YouTube Channel", url="https://youtube.com/@Tech_VJ")]]))
        ...
        if Config.UPDATES_CHANNEL:
            ...
            try:
                ...
                if user.status == "kicked":
                    await cmd.message.edit(text="Sorry Sir, You are Banned to use me. Contact my [𝙎𝙪𝙥𝙥𝙤𝙧𝙩 𝙂𝙧𝙤𝙪𝙥](https://t.me/VJ_Bot_Disscussion).", disable_web_page_preview=True)
                    return
        ...
        await cmd.message.edit("File Saved in Batch!\n\nPress below button to get batch link.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Get Batch Link", callback_data="getBatchLink")], [InlineKeyboardButton("Close Message", callback_data="closeMessage")]]))
    elif "addToBatchFalse" in cb_data:
        await save_media_in_channel_local(bot, editable=cmd.message, message=cmd.message.reply_to_message)
        ...
        await save_batch_media_in_channel_local(bot=bot, editable=cmd.message, message_ids=message_ids)
        MediaList[f"{str(cmd.from_user.id)}"] = []
